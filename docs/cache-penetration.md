# 존재하지 않는 게시글 반복 조회와 콜드 미스 폭주 방지

## 문제

게시글 상세 조회는 Redis 조회 모델에 값이 없으면 Article Service에서 원본 게시글을 조회한다. 삭제된 링크나 봇이 존재하지 않는 같은 ID를 반복 요청해도 `값이 없음`은 저장되지 않아, 모든 요청이 Article Service와 MySQL까지 전달됐다.

초기에는 100 req/s로 30초간 재현했다. 원본 조회는 3,001건이었지만 p95는 9.75ms라 지연 문제로 보기 어려웠고, 부재 캐시 적용 후 원본 조회가 2건으로 줄어 최초 조회 잠금은 제외했다.
...
동일한 존재하지 않는 게시글 ID에 2,000 req/s를 30초간 요청했다. JVM 초기화 영향을 줄이기 위해 다른 ID로 먼저 예열하고, 측정 직전 대상 ID의 부재 값과 잠금 키만 삭제했다.
...
![목표 요청률과 실제 처리율](images/cache-penetration/modu-square-cache-throughput.png)

- 변경 전에는 목표 2,000 req/s 중 1,856 req/s만 처리
- 적용 후에는 60,001건을 모두 처리하며 목표 요청률 유지
...
![시작하지 못한 요청](images/cache-penetration/modu-square-cache-dropped.png)
...
- 한 개의 존재하지 않는 ID에 요청이 집중되는 조건으로 콜드 미스와 캐시 관통을 재현
