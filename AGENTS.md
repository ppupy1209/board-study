...
이벤트 기반 커뮤니티 서비스. 대규모 게시판에서 자주 쓰이는 기능(게시글·댓글·좋아요·조회수·인기글·알림·이미지·인증)을
...
- Spring Boot 3.5.4 / Spring Web / Spring Data JPA / Spring Security / Spring Kafka / Spring Data Redis
- Lombok, JUnit 5
...
hot-article-list / media-upload / auth-refresh-replay)는 [load-tests/k6/](load-tests/k6) 참고.

## 아키텍처 결정 로그

| 날짜 | 결정 | 이유 |
|---|---|---|
| 2026-09-01 | auth: Refresh Token Rotation + Token Family + 재사용 탐지 | 로그아웃 뒤 탈취 Token 재사용과 동시 갱신 분기를 차단하고, 탐지 시 신규 Token까지 패밀리 전체를 폐기하기 위해 |
...
- **스코프**는 모듈명을 그대로 쓴다 (`article`, `article-read`, `hot-article`, `media`, `notification`, `auth`, `web`, `infra`).
...
- **완료**
  - 9개 서비스 모듈 + 4개 common 모듈 구성, Docker Compose 단일 명령 기동
  ...
  - notification 묶음 알림
  - auth Refresh Token 회전·재사용 탐지·패밀리 폐기, Testcontainers 동시성 통합 테스트
  - Prometheus/Grafana 관측성, k6 부하 테스트 스위트 11종
  - Java 21 Gradle 빌드 환경 정리 (2026-08-07) — `compileJava compileTestJava` 전 모듈 통과 확인

- **진행 중**
  - (없음)

- **다음 작업** (docs/development-log.md "12. 다음 개선" 기준)
  - Query Model 전체 재구축과 체크포인트
  - Kafka 파티션 확대 시 key ordering 검증
  - OAuth2/OIDC 공급자 연동과 가짜 OIDC 서버 callback 통합 테스트
