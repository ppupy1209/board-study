...
상단의 `Guest`를 누르면 로그인·회원가입 화면으로 이동할 수 있습니다. 회원가입을 마치면 같은 이메일로 로그인하고, 로그인 뒤 표시되는 이름을 눌러 이메일과 로그아웃 메뉴를 확인할 수 있습니다. 새로고침할 때는 `HttpOnly` Refresh Token으로 로그인 상태를 복구합니다.
