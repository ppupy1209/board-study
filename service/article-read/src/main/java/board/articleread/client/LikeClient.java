    ...
    public Map<Long, Long> countAll(List<Long> articleIds) {
        ...
        try {
            Map<Long, Long> result = restClient.get()
                    .uri(uriBuilder -> uriBuilder.path("/v1/article-likes/articles/counts")
            ...
            log.error("[LikeClient.countAll] articleIds size = {}", articleIds.size(), e);
            return Map.of();
        }
    }
}
